...
class Message extends CI_Model {
    ...
    function get_mails( $filters = [] )
    {
        ...
        if ($query->num_rows() > 0)
        {
            ...
            foreach($messages as $message)
            {
                ...
                $message->hours_ago = time_ago($message->send_date);
                $message->str_timestamp = date("M d, Y", strtotime($message->send_date));                
            }
            return $messages;
        }
    ...
    function get_alerts( $filters = [] )
    {
        ...
        $this->db->select("l.*, CONCAT(p.first_name, ' ', p.last_name) client_name, p.*");
        ...
        if ($query->num_rows() > 0)
        {
            ...
            {
                $alert = $row;
                $alert->header = " has pending loan application";
                $alert->client_name = $row->client_name;
                $alert->hours_ago = time_ago(date("Y-m-d H:i:s", $row->loan_applied_date));
                $alert->str_timestamp = date("M d, Y", $row->loan_applied_date);
                
                $image_url = base_url("uploads/profile-" . $alert->customer_id . "/" . $alert->photo_url);
                $headers = @get_headers($image_url, 1); // @ to suppress errors. Remove when debugging.
                if (isset($headers['Content-Type']))
                {
                    if (strpos($headers['Content-Type'], 'image/') === FALSE)
