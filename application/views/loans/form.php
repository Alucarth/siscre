...
$(document).ready(function() {
    ...
    var currentStatus = '<?php echo isset($loan_info) ? $loan_info->loan_status : ""; ?>';
    var statusSelect = $('select[name="status"]');
    
    // Deshabilitar opción de aprobado si el préstamo está rechazado
    if (currentStatus === 'rejected') {
        statusSelect.find('option[value="approved"]').prop('disabled', true);
        
        // Mostrar mensaje al usuario
        statusSelect.after(
            '<div class="alert alert-warning mt-2">'+
            '<i class="fas fa-exclamation-triangle"></i> '+
            '<?php echo $this->lang->line("loans_rejected_warning"); ?>'+
            '</div>'
        );
    }
    ...
    $('form').submit(function() {
        if (currentStatus === 'rejected' && statusSelect.val() === 'approved') {
            alert('<?php echo $this->lang->line("loans_cannot_approve_rejected"); ?>');
            return false;
        }
        return true;
    });
