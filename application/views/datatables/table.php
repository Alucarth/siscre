<style>
    ...
    <?php if( isset($object->no_expand_height) ): ?>
    
    <?php else: ?>
        <?= $object->table_id ?>_wrapper .dataTables_scrollBody {
            max-height: <?= $object->dt_height ?> !important;
            height: <?= $object->dt_height ?> !important;
        }
    <?php endif; ?>

    <?= $object->table_id ?> .dataTables_scroll {overflow:auto}
                    ...
                    <?php if($key == 'data_type' && $value == 'group_select'): ?>
                        ...
                        DataTable.ext.editorFields.<?=$field->get_fields()->type;?> = {
                            create: function ( conf ) {
                                ...
                                <?php if ( isset($field->get_fields()->no_group) && $field->get_fields()->no_group ): ?>
                                    <?php foreach($field->get_fields()->options as $option_key => $option_value): ?>
                                        $select_html += '<option value="<?=$option_value['value'];?>"><?=$option_value['label'];?></option>';
                                    <?php endforeach; ?>
                                <?php else: ?>                                    
                                    <?php foreach($field->get_fields()->options as $option_key => $option_value): ?>
    ...
    $(document).ready(function () {
        ...
        myTable = $('<?=$object->table_id; ?>').DataTable({
            ...
            initComplete: function(settings, json){                
                ...
                $(".dt-custom-button").appendTo('<?= $object->table_id ?>' + "_wrapper .dt-buttons");
		$("<?= str_replace("#", ".", $object->table_id); ?>_dt-custom-button").appendTo("<?= $object->table_id ?>_wrapper .dt-buttons");
            },
            drawCallback: function(settings) {
                ...
                $(".dataTables_scrollHeadInner .dt-min-width").each(function(){
                    var result = $(this).attr("style").split(";");
                    if ( result.length > 0 )
                    {
                        if ( result[1] == null ) // check if result[1] is undefined
                        {
                        }
                        else
                        {
                            var width = result[1].split(":");
