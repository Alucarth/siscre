...
<div class="extra-filters" style="display: none;">
    ...
    <!--&nbsp;<button class="btn btn-primary" id="btn-export-pdf"><span class="fa fa-table"></span> Exportar PDF</button>-->
...
<script>
    $(document).ready(function () {
        ...
        $(document).on("click", "#btn-save-extra-field", function(){
            ...
            if ( params.length == 0 )
            {
                alertify.alert("Agregue una fila");
                return false;
            }
            var has_error = false;
            ...
            if ( has_error )
            {
                alertify.alert("El cuadro de entrada no puede estar vacío. porfavor introduzca un valor");
                return false;
            }
            params += '&softtoken=' + $("input[name='softtoken']").val();
            params += '&type=3';
            $.post(url, params, function(data){
                if ( data.status == "OK" )
                {
                    alertify.alert("¡Los campos adicionales se han guardado correctamente!", function(){
        ...
        $("#tbl_borrowers_filter input[type='search']").attr("placeholder", "Escriba su búsqueda aquí");
        ...
        $(document).on("click", ".btn-delete", function () {
            var $this = $(this);
            alertify.confirm("¿Está seguro de que desea eliminar este cliente?", function () {
