                        ...
                        <table class="table table-hover table-bordered" id="tbl_liability">
                            <thead>
                                <tr>
                                    ...
                                    <th style="text-align: center">Importe pagable</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content" style="width:600px">
            <div class="modal-header">
                 Pasivo
                <input type="hidden" name="transaction_type" id="account_type" value="liability" />
            </div>
...
<script>
    $(document).ready(function () {
        $("#tbl_liability_filter").prepend("<a href='javascript:void(0)' class='btn btn-primary pull-left' id='btn-new-liability'>Crear</a>");
        $("#tbl_liability_filter input[type='search']").attr("placeholder", "Escriba su busqueda");
        $("#tbl_liability_filter input[type='search']").removeClass("input-sm");
        
        $("#btn-save-liability").click(function(){
            var url = '<?=site_url('accounting/ajax');?>';
            var params = $("#md-liability input, #md-liability textarea, #md-liability select").serialize();
            ...
            $.post(url, params, function(data){
                if ( data.status == "OK" )
                {
                    ...
                    $("#tbl_liability").DataTable().ajax.reload();
                }
                else
                {
        ...
        $(document).on("click", "#btn-new-liability", function(){            
            $("#md-liability .modal-body input, #md-liability .modal-body textarea").val("");
            $("#md-liability #code_number").prop("disabled", false);
            ...
            $.post(url, params, function(data){
                if ( data.status == "OK" )
                {
                    $.each(data.row, function(key, value){                        
                        $("#md-liability #" + key).val(value);
                    });
                    
                    $("#md-liability #code_number").prop("disabled", true);
                    $("#md-liability").modal("show");
                }
                else
                {
        ...
        $(document).on("click", ".btn-delete", function () {
            var $this = $(this);
            alertify.confirm("Esta seguro que desea eliminar este pasivo?", function () {
