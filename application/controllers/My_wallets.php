...
class My_wallets extends Secure_area implements iData_controller {
    ...
    {
        $datatable->add_server_params('', '', [$this->security->get_csrf_token_name() => $this->security->get_csrf_hash(), "type" => 1, "employee_id" => $this->input->get("employee_id")]);
    ...
    function _dt_wallets()
    {
        ...
        $from_date = $this->input->post("trans_from_date");
        $to_date = $this->input->post("trans_to_date");
        ...
        $this->_clear_deleted_payments();
        
        $filters = [];
        $filters["trans_date_from"] = $this->config->item('date_format') == 'd/m/Y' ? strtotime(uk_to_isodate($from_date)) : strtotime($from_date);
        $filters["trans_date_to"] = $this->config->item('date_format') == 'd/m/Y' ? strtotime(uk_to_isodate($to_date) . "+1 day") : strtotime($to_date . "+1 day");
    ...
    private function _clear_deleted_payments()
    {
