...
class Employees extends Person_controller {
    ...
    function _dt_employees()
    {
        ...
        foreach ($people->result() as $person)
        {
            ...
            if ( check_access($user_info->role_id, "employees", 'delete') )
            {
    ...
    function view($employee_id = -1)
    {
        ...
        if (is_plugin_active("branches") )
        {
            ...
            $data["branch_ids"] = json_decode($data["person_info"]->branch_ids, true);
        }
    ...
    private function _is_exists($username, $employee_id)
    {
    ...
    function save($employee_id = -1)
    {
        ...
        if ( $this->_is_exists($this->input->post("username"), $employee_id) )
        {
        ...
        if (is_plugin_active("branches"))
        {
            $employee_data["branch_ids"] = json_encode($this->input->post("branch_ids"));
        }
        
        $employee_data["can_approve_loan"] = $this->input->post("can_approve_loan") ? 1 : 0;
